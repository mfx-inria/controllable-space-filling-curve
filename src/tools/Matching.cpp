...
void Matching::augmentPath(int idx) {
	...
	while (!found) {
		if(childs.empty()) THROW_ERROR("Matching failed");
...
void Matching::checkInit() {
	if(_originalLinks.empty()) THROW_ERROR("Empty graph!");
	...
	if(!(maxLink == 3 && minLink >= 2)) THROW_ERROR("Graph is not 3-connected!");
}
