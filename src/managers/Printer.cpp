...
/*void Printer::writeResults(const GeneticAlgorithm &ga, const std::string &name)
{
    std::fstream myfi;
    myfi.open("results.txt", std::ios::app);
    if (Globals::_objective == MULTIMAT)
        myfi << "MULTIMAT";
    else if (Globals::_objective == VECTOR_FIELD)
        myfi << "VECTOR_FIELD";
    else if (Globals::_objective == ANISOTROPY)
        myfi << "ANISOTROPY";
    if (Globals::_objective == MINIMISE)
        myfi << " MINIMISE ";
    else if (Globals::_objective == MAXIMISE)
        myfi << " MAXIMISE ";
    for (const auto &layer : ga._layers)
    {
        myfi << "initial Score = " << layer.getInitialScore() << ", score = " << layer.getScore() << std::endl;
    }
    myfi.close();
}*/

// Write a simple .border file on this format
// shapes nb
// (for each shape)
// ... points nb
// ... (for each point)
...
// ... ... (for each hole point)
// ... ... ... X Y
void Printer::writeBorders(const std::vector<Layer> &layers, const std::string &name) {
    for(int i = 0; i < layers.size(); ++i) {
        std::fstream myfi;
        myfi.open("../script/measure/data/" + name + "_" + std::to_string(i) + ".borders", std::ios::out);
        myfi << layers[i]._operators.size() << '\n';
        for(const auto &op : layers[i]._operators)
        {
            auto border = op.getBorder();
            myfi << border._points.size() << '\n';
            for(const glm::vec2 &point : border._points)
                myfi << point.x << ' ' << point.y << '\n';
            myfi << border._holes.size() << '\n';
            for(const std::vector<glm::vec2> &hole : border._holes) {
                myfi << hole.size() << '\n';
                for(const glm::vec2 &point : hole)
                    myfi << point.x << ' ' << point.y << '\n';
            }
        }
        myfi.close();
...
// Write a simple .graph file on this format
// nb cycles
// (for each cycle)
// ... nb point inside
// ... (for each point)
// ... ... X Y radii
void Printer::writeToFile(const std::vector<Layer> &layers, const std::string &name, bool doVary) {
    for(int i = 0; i < layers.size(); ++i) {
        std::fstream myfi;
        myfi.open("../script/measure/data/" + name + "_" + std::to_string(i) + ".graph", std::ios::out);
        myfi << layers[i]._operators.size() << '\n';
        for(const LocalOperator &op : layers[i]._operators) {

            const auto &[points, zones] = op.getFinal();

            myfi << points.size() << '\n';
            if (doVary)
                for(const glm::vec3 &point : points)
                    myfi << point.x << ' ' << point.y << ' ' << point.z << '\n';
        }
        myfi.close();
    }
}
...
// ... (for each point)
// ... ... X Y radii
void Printer::writeLayersToFile(const std::vector<Layer> &layers) {
    std::fstream myfi;
    myfi.open("data.layer", std::ios::out);
    myfi << "SVGSIZE = " << Globals::_SVGSize.x << " " << Globals::_SVGSize.y << std::endl;
    for(int i = 0; i < layers.size(); ++i) {
        myfi << "layer" << std::endl;
        for(const LocalOperator &op : layers[i]._operators)
        {
            myfi << "op" << std::endl;
            myfi << "area" << std::endl;
            myfi << op.getArea() << std::endl;
            myfi << "succes" << std::endl;
            myfi << op.isSucces() << std::endl;
            const auto &zones = op.getZones();
            myfi << "zones" << std::endl;
            for (const auto &zone : zones)
                myfi << zone._strokeColor << std::endl;
            myfi << "zone" << std::endl;
            for (int z : op._zone)
                myfi << z << std::endl;

            myfi << "points" << std::endl;
            for(const glm::vec2 &point : op.getPoints())
                myfi << point.x << ' ' << point.y << '\n';
        }
        myfi.close();
    }
